...
# ==== Funkcja ataku i testowania ====
def attack(loader, epsilon, show_examples=False):
    model_attacker.eval()
    model_victim.eval()
    ...
    with torch.no_grad():
        ...
        acc_fgsm = (out_fgsm.argmax(1) == labels).sum().item() / labels.size(0) * 100

        out_pgd = model_victim(adv_pgd)
        acc_pgd = (out_pgd.argmax(1) == labels).sum().item() / labels.size(0) * 100

    print(f"Epsilon = {epsilon:.2f} | FGSM acc = {acc_fgsm:.2f}% | PGD acc = {acc_pgd:.2f}%")
    ...
    if show_examples:
        ...
        for i in range(6):
            ...
            plt.axis("off")
        plt.suptitle(f"Adversarial examples (epsilon={epsilon})")
        plt.tight_layout()
        plt.show()
...
# ==== Przeprowadź ataki dla różnych epsilonów ====
epsilons = [0.05, 0.1, 0.2, 0.5]
for eps in epsilons:
    attack(testloader, epsilon=eps, show_examples=True)